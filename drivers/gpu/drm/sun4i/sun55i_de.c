...
/* write a u32 into a block's shadow and mark its RCQ head dirty */
...
/*
 * Program the per-mode state: de_top control (MMIO) + datapath sizes (shadow).
 * Mirrors the vendor de_rtmx_mgr_apply enable path for sun55iw3.
 */
			...
			const struct drm_display_mode *mode)
{
	 ...
	 * RMW so we only own phys6's routing; the bootloader left the rest.
	 */
			    ...
			    struct drm_plane_state *state)
{
	...
	/* this milestone only drives the first UI channel (phys 6) */
	if (layer->type != SUN8I_LAYER_TYPE_UI ||
	    layer->channel != SUN55I_DE_OVL_UI_PHYS_CHN)
		return;
	...
	de_blk_write(lay, SUN55I_OVL_LAY_MBSIZE, SUN55I_DE_OUT_SIZE(src_w, src_h));
