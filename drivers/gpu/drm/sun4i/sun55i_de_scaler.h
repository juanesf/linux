...
/*
 * Allwinner v35x (DE33) VSU8 polyphase scaler coefficients and phase-set
 * selection. The 32-phase lanczos2 table and the zoom-region layout are taken
 * verbatim from the vendor BSP (disp2 lowlevel_v35x/de35x: de_scaler_table.c
 * lan2coefftab32[], de_vsu.c de_vsu_calc_fir_coef()).
 */

#ifndef _SUN55I_DE_SCALER_H_
#define _SUN55I_DE_SCALER_H_

#include <linux/types.h>

/* fixed-point widths used by the VSU step/phase registers (de_vsu.c) */
#define SUN55I_VSU_STEP_FRAC_BITS	19
#define SUN55I_VSU_PHASE_FRAC_BITS	19
#define SUN55I_VSU_PHASE_NUM		32

/* zoom-region sizes (in phase-sets) within the coefficient table */
#define SUN55I_VSU_ZOOM0_SIZE		1
#define SUN55I_VSU_ZOOM1_SIZE		8
#define SUN55I_VSU_ZOOM2_SIZE		4
#define SUN55I_VSU_ZOOM3_SIZE		1
#define SUN55I_VSU_ZOOM4_SIZE		1
#define SUN55I_VSU_ZOOM5_SIZE		1

static const u32 sun55i_vsu8_lan2_coef[512] = {
...
/*
 * Pick the coefficient phase-set offset (in table entries) for a given
 * horizontal/vertical step (de_vsu_calc_fir_coef). step is in
 * SUN55I_VSU_STEP_FRAC_BITS fixed point (1.0 == 1 << 19).
 */
static inline u32 sun55i_vsu8_coef_index(u32 step)
{
	u32 ratio = step >> (SUN55I_VSU_PHASE_FRAC_BITS - 3);
	u32 int_part = ratio >> 3;
	u32 frac = ratio & 0x7;
	u32 ofst;

	if (int_part == 0)
		ofst = SUN55I_VSU_ZOOM0_SIZE;
	else if (int_part == 1)
		ofst = SUN55I_VSU_ZOOM0_SIZE + frac;
	else if (int_part == 2)
		ofst = SUN55I_VSU_ZOOM0_SIZE + SUN55I_VSU_ZOOM1_SIZE + (frac >> 1);
	else if (int_part == 3)
		ofst = SUN55I_VSU_ZOOM0_SIZE + SUN55I_VSU_ZOOM1_SIZE +
		       SUN55I_VSU_ZOOM2_SIZE;
	else if (int_part == 4)
		ofst = SUN55I_VSU_ZOOM0_SIZE + SUN55I_VSU_ZOOM1_SIZE +
		       SUN55I_VSU_ZOOM2_SIZE + SUN55I_VSU_ZOOM3_SIZE;
	else
		ofst = SUN55I_VSU_ZOOM0_SIZE + SUN55I_VSU_ZOOM1_SIZE +
		       SUN55I_VSU_ZOOM2_SIZE + SUN55I_VSU_ZOOM3_SIZE +
		       SUN55I_VSU_ZOOM4_SIZE;

	return ofst * SUN55I_VSU_PHASE_NUM;
}

#endif /* _SUN55I_DE_SCALER_H_ */
