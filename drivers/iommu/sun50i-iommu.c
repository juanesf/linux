 ...
 * 0x0 - 0x40000000 (a hole from the IOMMU's point of view, as no DRAM
			    ...
			    int prot, gfp_t gfp, size_t *mapped)
{
	...
	/*
	 * The IOMMU's input and output addresses are 32 bits wide, but pages
	 * above 4 GiB (on 4 GiB boards, where DRAM spans 0x40000000 -
	 * 0x140000000) are reachable through the interconnect's alias of the
	 * sub-DRAM-base window, see sun50i_mk_pte(). Only reject what lies
	 * beyond even that.
	 */
	if (paddr >= SZ_4G + SUN50I_IOMMU_DRAM_BASE) {
		ret = -EINVAL;
		dev_warn_once(iommu->dev,
			      "attempt to map address beyond the DRAM alias window\n");
		goto out;
	}
